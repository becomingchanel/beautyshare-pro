...
export async function POST(req: Request) {
  try {
    ...
    const { email, full_name, first_name, last_name, business_name, id: ghlContactId } = body;
    ...
    const existing = users?.find((u: any) => u.email === normalized);
    let userId: string;
    ...
    if (existing) {
      ...
      const { data: newUser, error: createErr } = await createAdminClient().auth.admin.createUser({
        email:         normalized,
        email_confirm: true,
    ...
    // Generate welcome magic token (30 min expiry)
